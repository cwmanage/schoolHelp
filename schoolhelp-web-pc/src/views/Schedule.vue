  ...
  <div class="schedule-page">
    ...
    <div class="semester-bar">
      <div class="sem-left">
        ...
        <span class="week-badge" v-if="viewWeek">第 {{ viewWeek }} 周<span v-if="weekOffset !== 0" class="week-off">（视图）</span></span>
        <el-button-group v-if="week1Date" class="week-nav">
    ...
    <el-dialog v-model="detailVisible" title="课程详情" width="460px">
      ...
      <template #footer>
        ...
        <el-button v-if="detailItem && (detailItem.courseId || isInLib)" @click="goCourseLib">
          课程详情
        </el-button>
    ...
    <el-dialog v-model="tcVisible" title="作息设置（每节课时间）" width="560px" top="6vh">
      <div class="tc-bar">
        ...
        <span class="tc-gap-label">每节时长(分钟)</span>
        <el-input-number v-model="tcDuration" :min="20" :max="120" size="small" style="width: 100px" @change="recalcFrom(0)" />
      ...
      <div class="tc-week1">
        ...
        <span class="tc-week1-tip">改上课时间会自动按下课时长计算下课时间；后面的节次按课间间隔顺延</span>
      </div>
      <div class="tc-list">
...
import { Plus, Camera, Timer, ArrowLeft, ArrowRight, UploadFilled } from '@element-plus/icons-vue'
...
/** 是否视图周上课：当前真实周非本周课虚化；翻页视图同样规则（保持一致） */
 ...
 * - 改「结束时间」：本节时长以新结束为准，后续节按课间顺延（时长跟随 tcDuration）
 */
